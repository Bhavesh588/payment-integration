...
function PaymentPage() {
    ...
    useEffect(() => {
        axios.get("http://localhost:5000/.netlify/functions/api/paymentreceipt").then((res_payment) => setAllPayment(res_payment.data));
    }, []);
    ...
    const onSubmit = async (values) => {
        ...
        if (values.gateway === "PayU") {
            ...
            await axios.get("http://localhost:5000/.netlify/functions/api/ngenius").then(async (response) => {
                ...
                await axios
                    .post(`http://localhost:5000/.netlify/functions/api/ngenius/order`, {
        ...
        <div>
            ...
            <form action="https://test.payu.in/_payment" method="post">
                ...
                <input
                    type="hidden"
                    name="surl"
                    // value={`https://payment-api252.netlify.app/.netlify/functions/api/paymentreceipt/success?customer_id=${formref.current ? formref.current?.values.customer_ID : ""}&gateway=PayU`}
                    value={`http://localhost:5000/.netlify/functions/api/paymentreceipt/success?customer_id=${
                ...
                <input
                    type="hidden"
                    name="furl"
                    // value={`https://payment-api252.netlify.app/.netlify/functions/api/paymentreceipt/failure?customer_id=${formref.current ? formref.current?.values.customer_ID : ""}&gateway=PayU`}
                    value={`http://localhost:5000/.netlify/functions/api/paymentreceipt/failure?customer_id=${
                        formref.current ? formref.current?.values.customer_ID : ""
                    }&gateway=PayU`}
                />
                    ...
                    <tbody>
                        {all_payment?.map((pay, i) => (
                            <tr key={i}>
                                ...
                                <td>
                                    <span
                                        className={`${pay.Status === "success" ? "bg-success" : "bg-danger"} text-light`}
